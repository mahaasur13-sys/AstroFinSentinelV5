...
class GlobalInvariantEngine:
    ...
    def evaluate(
        ...
        if curr < prev:
            violations.append(
                f"COMMIT_INDEX_REGRESSION: prev={prev}, curr={curr} (DESC commit index MUST be monotonic)"
            )
        ...
        if self.spec.enable_temporal_strictness:
            ...
            if max_skew > self.spec.clock_skew_threshold_ms:
                violations.append(
                    f"TEMPORAL_DRIFT: max_skew={max_skew:.1f}ms (threshold={self.spec.clock_skew_threshold_ms:.1f}ms)"
                )
