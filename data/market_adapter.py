...
class MarketAdapter:
    ...
    def fetch_ohlcv(self, symbol: str, interval: str = "1h", limit: int = 100) -> list[OHLCV]:
        ...
        for i in range(limit):
            ...
            h, l = max(o,c)*(1+abs(change)*0.5), min(o,c)*(1-abs(change)*0.5)
            data.append(OHLCV(timestamp=ts, open=round(o,4), high=round(h,4), low=round(l,4), close=round(c,4), volume=random.uniform(1000,10000)))
            price = c
        return data
