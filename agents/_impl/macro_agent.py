...
class MacroAgent(BaseAgent[AgentResponse]):
    ...
    def _analyze_gold(self, gold: float, btc_price: float) -> tuple:
        ...
        if gold_btc < 0.05:
            return SignalDirection.LONG, 0.60, "Gold weak vs BTC (risk-on)"
        elif gold_btc > 0.1:
            return SignalDirection.NEUTRAL, 0.50, "Gold strong (inflation hedge)"
        else:
            return SignalDirection.NEUTRAL, 0.50, "Gold/BTC ratio neutral"
