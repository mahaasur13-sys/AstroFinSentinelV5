...
karll_validation:
  replay_buffer_ready: true
  oap_track: true
  supports_selfq: """ + str(agent.get("karl", {}).get("supports_selfq", True)).lower() + """
"""
...
╠══════════════════════════════════════════════════════════╣""")
    ...
    print(f"  KARL-integrated agents: {sum(1 for k in results['success'] if AGENTS[k].get('karl', {}).get('reward_weight', 0) > 0)}")
    print(f"  TTC-capable agents: {sum(1 for k in results['success'] if AGENTS[k].get('karl', {}).get('supports_ttc', False))}")
    print(f"  SelfQ-capable agents: {sum(1 for k in results['success'] if AGENTS[k].get('karl', {}).get('supports_selfq', False))}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", "-o", type=str, default="integrations/gitagent",
