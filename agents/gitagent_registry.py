...
AGENT_AGENTS: Dict[str, Dict] = {
    # Astro Domain
    "AstroCouncil": {
        "name": "AstroCouncil", "domain": "astro", "weight": 0.20,
        "karl": True, "ttc": True, "selfq": True,
        "path": "agents._impl.astro_council",
        "method": "run_astro_council",
    },
    "BradleyAgent": {
        "name": "BradleyAgent", "domain": "astro", "weight": 0.03,
        "karl": True, "ttc": True, "selfq": True,
        "path": "agents._impl.bradley_agent",
        "method": "run_bradley_agent",
    },
    "GannAgent": {
        "name": "GannAgent", "domain": "astro", "weight": 0.03,
        "karl": True, "ttc": True, "selfq": True,
        "path": "agents._impl.gann_agent",
        "method": "run_gann_agent",
    },
    "CycleAgent": {
        "name": "CycleAgent", "domain": "astro", "weight": 0.05,
        "karl": True, "ttc": True, "selfq": True,
        "path": "agents._impl.cycle_agent",
        "method": "run_cycle_agent",
    },
    "ElectoralAgent": {
        "name": "ElectoralAgent", "domain": "astro", "weight": 0.03,
        "karl": True, "ttc": False, "selfq": False,
        "path": "agents._impl.electoral_agent",
        "method": "run_electoral_agent",
    },
    "TimeWindowAgent": {
        "name": "TimeWindowAgent", "domain": "astro", "weight": 0.02,
        "karl": True, "ttc": True, "selfq": True,
    ...
    # Synthesis
    "SynthesisAgent": {
        "name": "SynthesisAgent", "domain": "synthesis", "weight": 0.00,
        "karl": True, "ttc": True, "selfq": False,
...
DOMAIN_WEIGHTS = {
    "fundamental": 0.20, "quant": 0.20, "macro": 0.15,
    "options": 0.15, "sentiment": 0.10, "technical": 0.10,
    "astro": 0.10, "research": 0.05, "risk": 0.00,
}

# TTC agents
TTC_AGENTS = {
    name for name, info in AGENT_AGENTS.items()
    if info.get("ttc", False)
}

# KARL agents
KARL_AGENTS = {
    name for name, info in AGENT_AGENTS.items()
    if info.get("karl", False)
}
...
class GitAgentRegistry:
    ...
    async def run(self, agent_name: str, input_state: Dict[str, Any],
                  use_ttc: bool = False) -> NormalizedOutput:
        """
        ...
        if not info:
            return NormalizedOutput(
                ...
                metadata={}, agent_name=agent_name, raw_output={},
            )
    ...
    async def _single_pass(self, agent_name: str,
                           input_state: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            if method is None:
                ...
                return {"signal": "NEUTRAL", "confidence": 50,
                        "reasoning": f"Method {method_name} not found", "metadata": {}}
            ...
            return {"signal": "NEUTRAL", "confidence": 50,
                    "reasoning": f"Module {agent_name} not importable", "metadata": {"error": "import"}}
        except Exception as e:
            return {"signal": "NEUTRAL", "confidence": 50,
                    "reasoning": f"Execution error: {e}", "metadata": {"error": str(e)}}

    async def _ttc_pass(self, agent_name: str,
                        input_state: Dict, k: int = 5) -> Dict[str, Any]:
        """
    ...
    async def run_all(self, input_state: Dict[str, Any],
                      domain_filter: Optional[str] = None,
                      use_ttc: bool = False) -> List[NormalizedOutput]:
    ...
    def weighted_consensus(
        ...
        weights = [self.get_info(o.agent_name).get("weight", 0.05)
                   for o in outputs]
