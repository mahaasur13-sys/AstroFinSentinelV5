...
from tools.metrics_server import RAG_RELEVANCE_SCORE, RAG_CHUNK_COUNT
...
def test_rag_query_cache_hits_increment():
    ...
    with patch('knowledge.rag_retriever._embed') as mock_embed:
        ...
        with patch.object(retriever, '_load') as mock_load:
            fake_chunks = [{"content": "c", "source": "s", "title": "t", "domain": "astrology", "relevance_score": 0.9}]
    ...
    with patch('knowledge.rag_retriever._embed') as mock_embed:
        ...
        with patch.object(retriever, '_load') as mock_load:
            fake_chunks = [
                {"content": "test1", "source": "s1", "title": "t1", "domain": "astrology", "relevance_score": 0.9},
                {"content": "test2", "source": "s2", "title": "t2", "domain": "astrology", "relevance_score": 0.7}
            ]
            mock_index = MagicMock()
            mock_index.ntotal = 2
