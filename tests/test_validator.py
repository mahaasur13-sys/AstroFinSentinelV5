...
class TestValidAgentYaml:
    ...
    def test_valid_with_temperature(self, validator, tmp_agent_dir):
        ...
        (agent_dir / "agent.yaml").write_text(yaml.dump({
            ...
            "version": "2.1.0",
            "model": {"provider": "anthropic", "name": "claude-sonnet-4", "temperature": 0.5},
    ...
    def test_valid_karllike_agent(self, validator, tmp_agent_dir):
        ...
        (agent_dir / "agent.yaml").write_text(yaml.dump({
            ...
            "capabilities": ["uncertainty_quantification", "amre_validation", "decision_record"],
            "tools": ["estimate_uncertainty", "validate_grounding", "build_decision_record"],
            "rules": [
            ...
            "compliance": {"karllike": True, "amre": True, "output_validation": True},
        }))
        result = validator.validate_file(agent_dir / "agent.yaml")
        assert result.valid is True
...
class TestInvalidAgentYaml:
    def test_missing_name(self, validator, tmp_agent_dir):
        ...
        (agent_dir / "agent.yaml").write_text(yaml.dump({
            ...
            "output_schema": {"signal": "S", "confidence": "C", "reasoning": "R"},
        }))
        result = validator.validate_file(agent_dir / "agent.yaml")
    ...
    def test_temperature_out_of_range(self, validator, tmp_agent_dir):
        ...
        (agent_dir / "agent.yaml").write_text(yaml.dump({
            ...
            "version": "1.0",
            "model": {"provider": "openai", "name": "gpt-4", "temperature": 3.0},
            "capabilities": ["test_capability"],
            "tools": [],
            "rules": ["Rule one for testing purposes"],
            "output_schema": {"signal": "S", "confidence": "C", "reasoning": "R"},
...
class TestDirectoryValidation:
    def test_validate_directory_non_recursive(self, validator, tmp_agent_dir):
        ...
        (tmp_agent_dir / "agent.yaml").write_text(yaml.dump({
            "name": "root_agent", "description": "Valid agent at root level",
            "version": "1.0", "model": {"provider": "openai", "name": "gpt-4"},
            "capabilities": ["test_cap"], "tools": [], "rules": ["Rule one testing"],
            "output_schema": {"signal": "S", "confidence": "C", "reasoning": "R"},
        }))
    ...
    def test_validate_directory_recursive(self, validator, tmp_agent_dir):
        ...
        for name in ["good_nested_1", "good_nested_2"]:
            ...
            (sub_dir / "agent.yaml").write_text(yaml.dump({
                "name": name.replace("_", "-"),
...
def test_print_report_quiet(capsys):
    ...
    r2.errors.append(ValidationIssue(
        path="name", value=None, expected="present",
        message="Required field missing", severity=Severity.ERROR,
    ))
