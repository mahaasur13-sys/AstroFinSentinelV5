...
def test_protected_endpoint_rejects_wrong_key(monkeypatch, client):
    ...
    resp = client.post("/api/live/enable", json={"confirmed": True}, headers={"X-API-Key": "wrong-key"})
...
def test_protected_endpoint_accepts_valid_key(monkeypatch, client):
    ...
    resp = client.post("/api/live/enable", json={"confirmed": True}, headers={"X-API-Key": "test-key"})
