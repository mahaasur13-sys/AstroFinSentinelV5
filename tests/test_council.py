...
class TestAstroCouncil:
    def test_majority_long(self):
        ...
        for _ in range(4):
            council.add_member(CouncilMember(name="a", domain="x", vote=Signal.LONG, confidence=60, reasoning="", weight=0.1, aligned=False))
        for _ in range(2):
            council.add_member(CouncilMember(name="b", domain="y", vote=Signal.SHORT, confidence=60, reasoning="", weight=0.1, aligned=False))
    ...
    def test_majority_short(self):
        council = AstroCouncil()
        for _ in range(3):
            council.add_member(CouncilMember(name="a", domain="x", vote=Signal.SHORT, confidence=70, reasoning="", weight=0.1, aligned=False))
        for _ in range(3):
            council.add_member(CouncilMember(name="b", domain="y", vote=Signal.NEUTRAL, confidence=60, reasoning="", weight=0.1, aligned=False))
    ...
    def test_weighted_signal(self):
        council = AstroCouncil()
        council.add_member(CouncilMember(name="a", domain="x", vote=Signal.LONG, confidence=100, reasoning="", weight=0.5, aligned=False))
        council.add_member(CouncilMember(name="b", domain="y", vote=Signal.LONG, confidence=60, reasoning="", weight=0.3, aligned=False))
        council.add_member(CouncilMember(name="c", domain="z", vote=Signal.SHORT, confidence=70, reasoning="", weight=0.2, aligned=False))
        result = council.vote("TEST", "test")
    ...
    def test_all_agents(self):
        agents = [
            ...
            AGENT_FACTORIES["astro"](price=100, predicted_return=0.02, moon_long=120, jupiter_long=200, saturn_long=300, nakshatra=14, choghadiya="Shubh", retrograde=[]),
        ]
        for a in agents:
    ...
    def test_run_council(self):
        ...
        result = run_council("BTCUSDT", price=48000, fair_value=52000, predicted_return=0.05, uncertainty=0.02, rsi=30, macd_bullish=True)
