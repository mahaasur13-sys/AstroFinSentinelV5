...
def test_faiss_load_cache_increments_counters():
    ...
    with patch('faiss.read_index', return_value=mock_index), \
         ...
         patch('pathlib.Path.read_text', return_value='[{"content":"test","source":"test.md","title":"Test"}]'), \
         patch('json.loads', return_value=fake_chunks):  # чтобы не читать реальный JSON
