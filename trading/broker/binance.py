...
class BinanceBroker(BaseBroker):
    ...
    def get_account_balance(self) -> AccountBalance:
        ...
        if not HAS_CCXT or not self.exchange:
            return AccountBalance(total_equity=10000.0, available_cash=10000.0, positions_value=0.0, total_pnl=0.0, currency="USDT")
        try:
            balance = self.exchange.fetch_balance()
            total_usd = float(balance.get("total", {}).get("USDT", 0))
            free_usd = float(balance.get("free", {}).get("USDT", 0))
            return AccountBalance(total_equity=total_usd, available_cash=free_usd, positions_value=total_usd-free_usd, total_pnl=0.0, currency="USDT")
        except Exception:
            BROKER_ERRORS.inc()
        ...
        try:
            ...
            for symbol, data in balance.get("total", {}).items():
                ...
                try:
                    ...
                    qty = float(data)
                    positions.append(Position(symbol=symbol, quantity=qty, avg_entry_price=0, current_price=price, unrealized_pnl=0, unrealized_pnl_pct=0))
                except Exception:
                    continue
            return positions
        except Exception:
    ...
    def place_order(self, symbol: str, side: OrderSide, order_type: OrderType,
        ...
        if not self.connected:
            return Order(order_id=order_id, symbol=symbol, side=side, order_type=order_type, quantity=quantity, price=price, status=OrderStatus.REJECTED, notes="Not connected")
        if not HAS_CCXT or not self.exchange or self.paper:
            ...
            return Order(order_id=order_id, symbol=symbol, side=side, order_type=order_type, quantity=quantity, price=price, status=OrderStatus.FILLED, filled_qty=quantity, avg_fill_price=exec_price, commission=commission, notes="Paper simulation" if self.paper else "Live (testnet)")
        try:
            ...
            return Order(order_id=str(order.get("id", order_id)), symbol=symbol, side=side, order_type=order_type, quantity=quantity, price=price, status=OrderStatus.FILLED, filled_qty=float(order.get("filled", quantity)), avg_fill_price=float(order.get("average", price or 0)), commission=float(order.get("fee", {}).get("cost", 0)), created_at=order.get("timestamp", time.time() * 1000) / 1000)
        except Exception as e:
            BROKER_ERRORS.inc()
            return Order(order_id=order_id, symbol=symbol, side=side, order_type=order_type, quantity=quantity, price=price, status=OrderStatus.REJECTED, notes=str(e))
    ...
    def get_order_status(self, order_id: str) -> Optional[Order]:
        ...
        try:
            ...
            return Order(order_id=str(order["id"]), symbol=order["symbol"], side=OrderSide.BUY if order["side"] == "buy" else OrderSide.SELL, order_type=OrderType.MARKET, quantity=float(order["amount"]), price=float(order.get("price", 0)), status=OrderStatus.FILLED if order["status"] == "closed" else OrderStatus.PENDING, filled_qty=float(order.get("filled", 0)))
        except Exception:
            BROKER_ERRORS.inc()
            return None
