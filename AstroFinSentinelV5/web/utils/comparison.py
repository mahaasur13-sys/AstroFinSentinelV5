...
def build_comparison_table(session_records: List[Dict]) -> str:
    ...
    for r in session_records:
        ...
        rows.append(f"<tr><td><code>{r.get('id','?')[:8]}</code></td>"
...
def build_comparison_chart(records_by_session: Dict[str, List[Dict]]) -> dict:
    ...
    for idx, (sid, records) in enumerate(records_by_session.items()):
        ...
        fig.add_trace(go.Bar(
            x=gens, y=rewards, name=f"Reward {label}",
...
def build_convergence_chart(gen_stats: List[Dict]) -> dict:
    ...
    fig.add_trace(go.Scatter(
        x=gens, y=max_rw, mode="lines+markers",
        name="Best", line=dict(color="#00d4ff", width=2),
        marker=dict(size=6),
    ), row=1, col=1)
    fig.add_trace(go.Scatter(
        x=gens, y=mean_rw, mode="lines+markers",
        name="Mean", line=dict(color="#ffd600", width=1.5, dash="dot"),
        marker=dict(size=5),
    ), row=1, col=1)
    fig.add_trace(go.Scatter(
        x=gens + gens[::-1],
        ...
        fill="toself", opacity=0.15, fillcolor="#ffd600",
    ...
    fig.add_trace(go.Histogram(
        y=rewards, name="Rewards",
        marker_color="#00d4ff", opacity=0.7,
        nbinsy=15, orientation="h",
    ), row=1, col=2)
