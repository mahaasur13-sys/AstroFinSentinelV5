...
class RiskEngineV2:
    def __init__(self, config: Optional[RiskConfigV2] = None):
    ...
    def get_state(self) -> RiskState:
        ...
        return RiskState(total_equity=equity, cash=equity, peak_equity=peak, current_drawdown=dd, kill_switch_active=kill)

    def check_kill_switch(self):
        state = self.get_state()
        if state.kill_switch_active:
            return False, state.current_drawdown, f"DRAWDOWN KILL: {state.current_drawdown:.2%} >= {self.config.max_drawdown:.2%}"
    ...
    def check_exposure(self, symbol, proposed_notional):
        ...
        if new_asset_exposure > self.config.max_exposure_per_asset:
            ...
            return False, max(0.0, scaled), f"EXPOSURE CAP: {new_asset_exposure:.2%} > limit"
    ...
    def pre_trade_check(self, symbol, proposed_notional, realized_vol=0.0, regime="NORMAL"):
        ...
        if not ok:
            ...
            return "REDUCED" if adjusted > 0 else "REJECTED", adjusted, f"CORRELATION: {msg}"
