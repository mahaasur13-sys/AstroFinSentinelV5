...
class TradingExecutionResult:
    ...
    reason: str
    adjusted_signal: Optional[Dict] = None
    ...
    def execute(self, strategy, market_data: Dict, mode: str = 'PAPER') -> TradingExecutionResult:
        ...
        try:
            ...
            if not mode_result.get('allowed', True):
                return TradingExecutionResult(status='REJECTED', reason=mode_result.get('reason', 'Mode gate rejected'))
            # 2) Pre-trade risk
            ...
            if not sanity.get('passed', True):
                return TradingExecutionResult(status='REJECTED', reason=sanity.get('reason', 'Sanity check failed'))
            # 5) Log
