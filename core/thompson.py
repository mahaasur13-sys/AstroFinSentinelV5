...
class ThompsonSampler:
    ...
    def _apply_oap_weighting(
        self,
        eligible: list[tuple[str, float, Optional[BeliefState]]],
        oap_adjustments: Optional[dict[str, float]],
    ) -> list[tuple[str, float, Optional[BeliefState]]]:
        """
        Apply OAP soft weighting:
        new_sample = max(0.1, base_sample * (1 + adjustment))

        Guarantees:
        - No agent fully suppressed (min weight = 0.1)
        - Backward compatible if no adjustments
        """
        ...
        for agent_name, sample, belief in eligible:
            adjustment = oap_adjustments.get(agent_name, 0.0)
            new_sample = max(0.1, sample * (1.0 + adjustment))

            if adjustment != 0.0:
                print(
                    f"[OAP] {agent_name}: {sample:.3f} → {new_sample:.3f} "
                    f"(adj={adjustment:+.3f})"
                )
