...
class AtomProposer:
    ...
    def _extract_keywords(self, finding) -> list[str]:
        ...
        important = [
            "multi-agent", "coordination", "pressure field", "temporal decay",
            "thompson sampling", "reward", "oap", "uncertainty",
            "mas factory", "topology", "switch node", "agent",
            "visualization", "dashboard", "backtest", "postgresql",
            "ensemble", "voting", "confidence", "calibration",
            "self-improvement", "meta-reasoning", "grounding",
        ]
    ...
    def save_proposals(self, output_path: str = None) -> str:
        ...
        for p in sorted(self.proposals, key=lambda x: (["P0", "P1", "P2"].index(x.priority), x.atom_id)):
            lines.extend([
                f"## {p.atom_id}: {p.title}",
                "",
