...
class MetaAgent:
    ...
    def initialize_population(self, generator_fn=None, market_data=None):
        ...
        for _ in range(self.config.population_size):
            ...
            scored = ScoredStrategy(
                strategy=strategy, reward=0.0, evaluation=EvaluationResult.fail(),
    ...
    def evaluate_population(self, population, market_data):
        from agents._impl.amre.audit import META_RL_DECISION_RECORD_ENABLED, record_meta_rl_decision
    ...
    def select(self, population):
        ...
        elites.sort(key=lambda s: s.reward_history[-1] if s.reward_history else s.reward, reverse=True)
    ...
    def evolve(self, elites, generator_fn=None):
        ...
        while len(new_pop) < self.config.population_size:
            ...
            if roll < adaptive_rate:
                ...
                child = GeneratedStrategy(chrom, generation=elites[0].generation + 1,
                ...
                new_pop.append(ScoredStrategy(
                    ...
                    generation=elites[0].generation + 1, parent_ids=(a.id, b.id)))
                ...
                new_pop.append(ScoredStrategy(
                    strategy=child, reward=0.0, evaluation=EvaluationResult.fail(),
                    generation=elites[0].generation + 1, parent_ids=(p.id,)))
            else:
                child = self._generate_random_strategy(generator_fn, elites[0].generation + 1)
                new_pop.append(ScoredStrategy(
                    strategy=child, reward=0.0, evaluation=EvaluationResult.fail(),
                    generation=elites[0].generation + 1, parent_ids=()))
        self._generation += 1
