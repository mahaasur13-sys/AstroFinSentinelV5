"""schema.py — Pydantic output schema for BullResearcher"""
...
class BullResearcherOutput(BaseModel):
    agent_name: str = Field(default="BullResearcher")
    signal: Literal["LONG", "SHORT", "NEUTRAL", "AVOID"]
    confidence: int = Field(..., ge=0, le=100)
    reasoning: str = Field(...)
    sources: List[str] = Field(default_factory=list)
    metadata: BullResearcherMetadata = Field(default_factory=BullResearcherMetadata)
