...
class AgentYamlValidator:
    ...
    def validate_file(self, yaml_path: Path) -> ValidationResult:
        ...
        # ── Required fields ──────────────────────────────────────────────────
        required_fields = ["name", "description", "version", "model", "capabilities", "tools", "rules", "output_schema"]
        ...
        if "capabilities" in data:
            if not isinstance(data["capabilities"], list):
                ...
                if len(data["capabilities"]) != len(set(str(c) for c in data["capabilities"])):
                    dupes = [c for c in data["capabilities"] if list(str(x) for x in data["capabilities"]).count(str(c)) > 1]
                    result.warnings.append(ValidationIssue(
                        path="capabilities",
                        value=dupes[:5],
    ...
    def print_report(self, report: ValidationReport, verbose: bool = False):
        ...
        print(f"     Total:   {report.total}   {c(f'Passed: {report.passed}', GREEN)}   {c(f'Failed: {report.failed}', RED if report.failed > 0 else GREEN)}   {c(f'Warnings: {report.warning_count}', YELLOW)}")
        print()
...
def run_validation(strict: bool = False, all_agents: bool = False, dir_path: Optional[str] = None):
    """CLI entry point."""
    try:
