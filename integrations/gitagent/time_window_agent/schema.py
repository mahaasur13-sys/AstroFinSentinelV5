"""schema.py — Pydantic output schema for TimeWindow"""
...
class TimeWindowOutput(BaseModel):
    agent_name: str = Field(default="TimeWindow")
    signal: Literal["LONG", "SHORT", "NEUTRAL", "AVOID"]
    confidence: int = Field(..., ge=0, le=100)
    reasoning: str = Field(...)
    sources: List[str] = Field(default_factory=list)
    metadata: TimeWindowMetadata = Field(default_factory=TimeWindowMetadata)
