"""schema.py — Pydantic output schema for BearResearcher"""
...
class BearResearcherOutput(BaseModel):
    agent_name: str = Field(default="BearResearcher")
    signal: Literal["LONG", "SHORT", "NEUTRAL", "AVOID"]
    confidence: int = Field(..., ge=0, le=100)
    reasoning: str = Field(...)
    sources: List[str] = Field(default_factory=list)
    metadata: BearResearcherMetadata = Field(default_factory=BearResearcherMetadata)
