"""schema.py — Pydantic output schema for MarketAnalyst"""
...
class MarketAnalystOutput(BaseModel):
    agent_name: str = Field(default="MarketAnalyst")
    signal: Literal["LONG", "SHORT", "NEUTRAL", "AVOID"]
    confidence: int = Field(..., ge=0, le=100)
    reasoning: str = Field(...)
    sources: List[str] = Field(default_factory=list)
    metadata: MarketAnalystMetadata = Field(default_factory=MarketAnalystMetadata)
