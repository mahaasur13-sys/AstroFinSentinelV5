...
def register_callbacks(app, get_engine_ref):
    ...
    def deploy_to_karl(n_clicks, strategy_data):
        ...
        try:
            ...
            return make_toast(
                f"Deploy error: {e}", "Deploy Failed", "danger"
            ), dash.no_update
    ...
    def paper_test(n_clicks, strategy_data):
        ...
        msg = (
            f"Paper test queued for {strategy_data.get('id', '?')[:8]}. "
    ...
    def export_json(n_clicks, strategy_data):
        ...
        return make_toast(
            f"Saved to data/exports/{safe}", "Export JSON", "success"
        ), True
    ...
    def export_python(n_clicks, strategy_data):
        ...
        with open(path, "w", encoding="utf-8") as f:
            f.write(py_code)
        return make_toast(
            f"Saved to data/exports/{safe}", "Export Python", "success"
        ), True
    ...
    def show_strategy_detail(sid, session_id):
        empty_fig = lambda: go.Figure().update_layout(
