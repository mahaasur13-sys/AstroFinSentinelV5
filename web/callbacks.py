...
def register_callbacks(app, get_engine_ref):
    ...
    def refresh_sessions_table(_):
        ...
        for sid in sessions:
            ...
            badge = "bg-success" if best > 0.7 else "bg-warning" if best > 0.4 else "bg-secondary"
            rows.append(html.Tr([
    ...
    def _build_equity_and_dd(ec):
        ...
        if ec and len(ec) > 1:
            ...
            dd_fig.add_trace(go.Scatter(
                y=-dd_pct, mode="lines", fill="tozeroy",
            ...
            dd_fig.add_annotation(
                x=int(np.argmax(dd_pct)), y=-max_dd,
        ...
        for f in (fig, dd_fig):
            f.update_layout(
                ...
                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                yaxis=dict(title="Equity" if f is fig else "Drawdown %",
    ...
    def start_evolution(n_clicks, symbol, timeframe, gens, pop, walk_forward):
        ...
        try:
            ...
            empty_fig = go.Figure().update_layout(template="plotly_dark", height=220)
            return (
                html.Div([dbc.Spinner(color="primary", size="sm"), html.Span(f" Running {gens} gens...")]),
                False, "0", "—", "—", "—", "—", "—", empty_fig, empty_fig
            )
        except Exception as e:
            logger.error(f"[DASH] Evolution start failed: {e}\n{traceback.format_exc()}")
            empty_fig = go.Figure().update_layout(template="plotly_dark", height=220)
            return (
                html.Div([html.Span("Error: ", className="text-danger"), html.Span(str(e)[:200])]),
                True, "—", "—", "—", "—", "—", "—", empty_fig, empty_fig
            )
    ...
    def poll_evolution(n_intervals, gens):
        ...
        if current_gen == 0:
            return (html.Span("Initializing..."), False, "0", "—", "—", "—", "—", "—",
        ...
        fig.add_trace(go.Scatter(x=gens_nums, y=max_rw, mode="lines+markers",
                                 name="Best", line=dict(color="#00d4ff", width=2)))
        fig.add_trace(go.Scatter(x=gens_nums, y=mean_rw, mode="lines+markers",
                                 name="Mean", line=dict(color="#ffd600", dash="dot")))
        fig.update_layout(template="plotly_dark", height=220, margin=dict(l=40, r=20, t=20, b=30),
                         paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                         yaxis=dict(title="Reward", showgrid=True, gridcolor="rgba(255,255,255,0.05)"))
        # Diversity chart
        div_fig = go.Figure()
        std_rw = [s.std_reward for s in hist]
        div_fig.add_trace(go.Bar(x=gens_nums, y=std_rw, name="Std Reward",
                                marker_color="#ffd600", opacity=0.7))
        div_fig.update_layout(template="plotly_dark", height=220, margin=dict(l=40, r=20, t=20, b=30),
                             paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
        ...
        if progress >= 100:
            ...
            return (status, True, str(current_gen), f"{best_r:+.4f}", f"{latest.mean_reward:+.4f}",
                    f"{latest.std_reward:.4f}", str(trades), f"{best_sharpe:.3f}", fig, div_fig)
        else:
            ...
            return (status, False, str(current_gen), f"{latest.max_reward:+.4f}",
                    f"{latest.mean_reward:+.4f}", f"{latest.std_reward:.4f}",
                    str(best_trades), f"{karl_q:+.3f}", fig, div_fig)
    ...
    def update_system_status(n):
        from meta_rl.config import LIVE_DATA_ENABLED, META_RL_ENABLED, WALK_FORWARD_ENABLED
        from meta_rl.persistence import get_persistence
        p = get_persistence()
        summary = p.get_sessions_summary()
        rows = [
            html.Tr([html.Td("Meta-RL"), html.Td("🟢 Active" if META_RL_ENABLED else "🔴 Disabled")]),
            html.Tr([html.Td("Live Data"), html.Td("🟢 Connected" if LIVE_DATA_ENABLED else "⚪ Sandbox")]),
            html.Tr([html.Td("Walk-Forward"), html.Td("🟢 Enabled" if WALK_FORWARD_ENABLED else "⚪ Disabled")]),
            html.Tr([html.Td("Sessions"), html.Td(f"{summary.get('total_sessions', 0)}")]),
            html.Tr([html.Td("Strategies"), html.Td(f"{summary.get('total_strategies', 0)}")]),
            html.Tr([html.Td("Best Reward"), html.Td(f"{summary.get('max_reward', 0):+.4f}", className="text-success")]),
        ]
        return dbc.Table(
            [html.Thead(html.Tr([html.Th("Component"), html.Th("Status")]))] + [html.Tbody(rows)],
            bordered=False, size="sm", color=None, style={"background": "transparent"}
        )
    ...
    def deploy_to_karl(n_clicks, strategy_data):
        ...
        try:
            ...
            if not record:
                return make_toast(f"Strategy {s_id[:8]} not found in session", "Deploy Failed", "danger"), dash.no_update
            ss = ScoredStrategy.from_dict(record)
            agent = MetaAgent()
            agent.pool.add(ss)
            karl_state = agent.update_karl([ss])
            qstar = karl_state.get("current_q_star", 0.0)
            return make_toast(f"Strategy {s_id[:8]} deployed to KARL. Q*={qstar:+.4f}", "Deploy Success", "success"), True
        except Exception as e:
    ...
    def paper_test(n_clicks, strategy_data):
        ...
        from web.utils.notifications import make_toast
        msg = (f"Paper test queued for {strategy_data.get('id','?')[:8]}. "
               "Live paper mode coming soon.")
    ...
    def show_strategy_detail(sid, session_id):
        empty_fig = lambda: go.Figure().update_layout(template="plotly_dark", height=220)
        if not sid or not session_id:
            return (html.Div("Select a strategy"), empty_fig(), empty_fig(),
                    empty_fig(), empty_fig(), empty_fig(), dash.no_update,
                    "\u2014", "\u2014", "\u2014", "\u2014", "\u2014", "\u2014", "\u2014",
        ...
        if not record:
            return (html.Div("Strategy not found"),) + (empty_fig(),) * 5 + (
                dash.no_update, "\u2014", "\u2014", "\u2014", "\u2014", "\u2014", "\u2014",
                html.Span("Not found", className="text-danger small"), {}, True, True, True, True, True)
        ...
        id_display = html.Div([
            ...
            html.Span(f"Gen {record.get('generation', '?')} \u2022 ", className="text-muted small"),
            html.Span(f"reward={reward_str}", className="text-success small fw-bold"),
        ])

        # Chromosome table
        rows = [
            html.Tr([html.Td(k, className="small text-muted"), html.Td(str(v), className="small font-monospace")])
        ...
        if rh:
            rh_fig.add_trace(go.Scatter(
                y=list(rh), mode="lines+markers",
                name="Reward", line=dict(color="#00d4ff", width=2), marker=dict(size=5),
            ))
        ...
        return (chrom_html, equity_fig, radar_fig, dd_fig, rh_fig, sig_fig,
                alpha, reward_str, sharpe_str, pnl_str, str(tr), wr_str, dd_str,
                karl_str, id_display, record, False, False, False, False, False)
    ...
    def _alpha_badge(risk_adj_pnl, sharpe, trades):
        ...
        score = 0
        if sharpe and sharpe > 1.5: score += 2
        elif sharpe and sharpe > 1.0: score += 1
        if trades and trades >= 10: score += 1
        if risk_adj_pnl and risk_adj_pnl > 0: score += 1
        if score >= 4:
...
def render_live_status() -> html.Div:
    ...
    try:
        ...
        regime_color = {"BULL": "success", "BEAR": "danger", "NEUTRAL": "secondary", "VOLATILE": "warning"}.get(regime, "secondary")
        return html.Div([
            dbc.Badge(f"REGIME: {regime}", color=regime_color, className="me-2"),
            dbc.Badge(f"MODE: {hc.get('mode', 'sandbox').upper()}", color="info", className="me-2"),
            dbc.Badge(f"BTC: ${price:,.2f}", color="light", className="me-2"),
            dbc.Badge(f"HALTH: {hc.get('status', 'OK')}", color="success" if hc.get("status") == "OK" else "danger"),
